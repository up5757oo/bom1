      ...
      data.addRows([
        ...
        if ($result->num_rows > 0) {
          ...
          while($row = $result->fetch_assoc()) {
            //applying start date to a variable where either open date or dependancy date could be used
            $start_date = $row["open_date"];
            //$start_date = $row["dependency_date"];

            //hard docing percent value to 0
            $percent = 0 ;
        ...
        var options = {
          //limitting the height to 600 pixels
          height: 600,
          gantt: {
